...
export class ProfessorRetrieveApplicationsComponent implements OnInit {
  ...
  displayedColumns: string[] = ["firstName", "lastName", "level", "courseNumber", "currentGpa", "gpaInSubject", "lookingForCoop", "notes", "status", "actions"];
  ...
  ngOnInit(): void {
    ...
    this.courses$.subscribe((courses) => {
      if (courses) {
        this.coursesTitleArray = _.uniq(courses.filter((course) => course.Instructors === this.professor).map((course) => course.Title));
      }
      this.applicationService.getAllApplications().then(applications => {
        ...
        for (let i = 0; i < this.coursesTitleArray.length; i++) {
          for (let j = 0; j < this.applications.length; j++) {
            if (this.coursesTitleArray[i] === this.applications[j].courseNumber) {
  ...
  assignStatusToTA(event: any, application: Application) {
    ...
    this.applicationService.updateApplicationForStudent(application).subscribe();
